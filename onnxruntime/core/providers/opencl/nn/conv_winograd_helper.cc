...
WinogradHelper::WinogradHelper(AllocatorPtr& cpu_alloc, int compute_unit, int kernel_size) : cpu_alloc_{cpu_alloc} {
  ...
  Tensor G_(DataTypeImpl::GetType<float>(), {kernel_size, wino_size_}, cpu_alloc_);
...
std::unique_ptr<Tensor> AllocWeightTensor(AllocatorPtr& cpu_alloc_,
  ...
  // return std::make_unique<Tensor>(DataTypeImpl::GetType<float>(), {16LL, coC4, ciC4, unit_ci, unit_co}, cpu_alloc_);
  return nullptr;
}
